...
10年分の平均気温から学習し、interval日間の過去のデータから、次の日の平均気温を予測するプログラム
    ...
    CSV_FILE_NAMEでcsvファイルの名前を指定
    CUT_YEARで分割基準の西暦を指定
    OUTPUT_DIRECTORYで画像ファイルの出力先を決定
    
Notes
-----
・元のプログラムで作成されている"kion10y.csv"と同じ形式のCSVファイルを前提としています。
    一応、異なる西暦と分割基準がセットされたデータを用いて軽くチェックは行っていますが、
...
class TemperaturePrediction:
    """
    気温のデータを格納し、過去のデータから未来の気温を予測
    
    Attributes
    ----------
    _temperature_df : pandas.core.frame.DataFrame
        日毎の気温が格納されたデータフレーム
    ...
    def __init__(self, csv_file_name):
        ...
        csv_file_name : str
            気温のデータが格納されたCSVファイル
        """
        self._temperature_df =  pd.read_csv(csv_file_name, encoding='utf-8')
    ...
    def _divide_df(self, cut_year):
        ...
        (train_year_df, test_year_df) : tuple
            分割したデータフレームが含まれたタプル
        """
        train_year = (self._temperature_df["年"] <= cut_year)
        test_year = (self._temperature_df["年"] > cut_year)
        
        train_year_df = self._temperature_df[train_year]
        test_year_df = self._temperature_df[test_year]
...
temperature_predictor = TemperaturePrediction(CSV_FILE_NAME)
x_list = []
average_error_list = []
max_error_list = []
...
for i in range(1, 18):
    print(f"interval: {i}")
    temperature_predictor.learn(interval=i, cut_year=CUT_YEAR)
    average_error, max_error = temperature_predictor.report()
    print("")
...
plt.savefig(f'./{OUTPUT_DIRECTORY}/Linear_AverageError{CUT_YEAR}.png')
plt.show()
...
plt.savefig(f'./{OUTPUT_DIRECTORY}/Linear_MaxError{CUT_YEAR}.png')